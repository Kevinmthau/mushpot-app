        ...
        {documents && documents.length > 0 ? (
          documents.map((doc) => (
            <Link
              key={doc.id}
              href={`/doc/${doc.id}`}
              className="group rounded-2xl bg-[var(--paper)] px-5 py-4 transition hover:shadow-[0_8px_22px_rgba(41,60,68,0.08)]"
            >
              <div className="flex min-w-0 items-end gap-3">
                <p
                  dir="auto"
                  className="min-w-0 pr-2 text-xl font-semibold leading-tight text-[var(--ink)]"
                >
                  {doc.title || "Untitled"}
                </p>
