...
The Next.js batch endpoint forwards requests to `get-shared-doc`. Each batch rechecks the current share token and `share_enabled`, parses the document's allowed references once, validates owner/document/path for each item, and calls Storage `createSignedUrls` once per populated bucket. Invalid or unreferenced paths return no URL. Authorized Storage failures can retry using the original single-media route. Browser batch requests have a 10-second deadline, including reading the response body; a stalled request is aborted and falls back to the single-media route so later batches can continue. Credentials remain in the Edge Function runtime.
...
Automated checks cover a batch's one document lookup/per-bucket signatures, duplicates and mixed invalid references, share revocation, partial Storage failures, bounded batch concurrency, stalled requests and response bodies, URL expiry, old-edge fallback, offscreen observations, changed media props, and no-JavaScript SSR markup. A live-account check should load a shared document with several images plus a video/poster, scroll to later media after five minutes, revoke sharing before requesting new media, and confirm the same behavior with JavaScript disabled. Automated tests and synthetic fixtures do not establish deployed latency.
